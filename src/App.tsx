...
export default function App() {
  ...
  const [apiKey, setApiKey] = useState(() => localStorage.getItem("codelens_openai_key") || "");
  ...
  const handleRunTrace = async () => {
    ...
    try {
      const response = await axios.post("/api/trace", { code, apiKey });
      ...
      setError(err.response?.data?.details || err.response?.data?.error || "Failed to connect to tracing server");
    } finally {
      setIsTracing(false);
    }
  };
  ...
  const handleApiKeyChange = (val: string) => {
    setApiKey(val);
    localStorage.setItem("codelens_openai_key", val);
  };
    ...
    <div className="flex flex-col h-screen bg-slate-950 overflow-hidden select-none">
      ...
      <header className="h-14 border-b border-border bg-sidebar-bg flex items-center px-6 justify-between shrink-0">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 bg-slate-800/50 border border-border px-2 py-1 rounded-md">
            <span className="text-[10px] text-slate-500 font-mono uppercase tracking-tighter">API Key</span>
            <input 
              type="password"
              value={apiKey}
              onChange={(e) => handleApiKeyChange(e.target.value)}
              placeholder="sk-..."
              className="bg-transparent border-none text-xs text-white w-40 outline-none focus:ring-0 placeholder:text-slate-600"
            />
          </div>
